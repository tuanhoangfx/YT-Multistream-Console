...
function extractVideoLinksFromDriveHtml(html) {
  ...
  const escapedFilePattern = /https:\\\/\\\/drive\.google\.com\\\/file\\\/d\\\/([A-Za-z0-9_-]{20,})/gi;
  match = escapedFilePattern.exec(text);
  while (match) {
    registerCandidate(match[1]);
    match = escapedFilePattern.exec(text);
  }
...
function isLikelyDriveFileId(value) {
  ...
  if (!/^[A-Za-z0-9_-]{20,64}$/.test(id)) return false;
  if (id.includes("trendtiktok") || id.includes("http")) return false;
  const alphaCount = (id.match(/[A-Za-z]/g) || []).length;
  const digitCount = (id.match(/[0-9]/g) || []).length;
  return alphaCount >= 6 && digitCount >= 2;
}

function driveHtmlRequiresSignIn(html) {
  const text = String(html || "");
  return /Sign in\s*to continue to Google Drive/i.test(text) || /accounts\.google\.com\/ServiceLogin/i.test(text);
}
...
function bindStreamingApi() {
  ...
  ipcMain.handle("drive:scan-folder", async (_event, payload = {}) => {
    ...
    if (signInRequired) {
      throw new Error("Drive folder requires sign-in. Set access to 'Anyone with the link (Viewer)' then scan again.");
    }

    return { ok: true, links: [], message: "No public video links found in this folder." };
  });
